...
from tests.chat.conftest import make_result


def test_context_blocks_are_numbered_from_one():
    sources = (make_result(), make_result(id=2, celex="32015R0757", citation="Article 5"))
...
def test_context_blocks_carry_the_chunk_text():
    context = format_context((make_result(text="A very specific clause."),))
    assert "A very specific clause." in context


def test_user_message_puts_context_before_the_question():
    message = build_user_message("What is the limit?", (make_result(),))
