...
async def test_sparql_failure_aborts_and_marks_run_failed(db_session, store, corpus_client):
    client, _ = corpus_client({"mrv": httpx.Response(500, text="down")}, {})
    with pytest.raises(httpx.HTTPStatusError):
        await ingest(db_session, client=client, topics=["mrv"], store=store)
...
async def test_completed_run_is_stamped_with_a_dated_corpus_version(
    ...
    report = await ingest(db_session, client=client, topics=["mrv"], store=store)
...
async def test_unchanged_corpus_keeps_the_previous_corpus_version(db_session, store, corpus_client):
    docs = mrv_docs()
    client, _ = corpus_client({"mrv": MRV_SPARQL}, docs)
    first = await ingest(db_session, client=client, topics=["mrv"], store=store)
    client, _ = corpus_client({"mrv": MRV_SPARQL}, docs)
    second = await ingest(db_session, client=client, topics=["mrv"], store=store)
...
async def test_changed_document_produces_a_new_corpus_version(db_session, store, corpus_client):
    client, _ = corpus_client({"mrv": MRV_SPARQL}, mrv_docs())
    first = await ingest(db_session, client=client, topics=["mrv"], store=store)
...
async def test_failed_run_is_not_stamped_with_a_corpus_version(db_session, store, corpus_client):
    client, _ = corpus_client(
...
async def test_a_completed_run_records_what_each_stage_did(db_session, store, corpus_client):
    client, _ = corpus_client({"mrv": MRV_SPARQL}, mrv_docs())
    report = await ingest(db_session, client=client, topics=["mrv"], store=store)
...
async def test_a_stage_failure_is_answerable_from_the_run_row(db_session, store, corpus_client):
    client, _ = corpus_client(
        {"mrv": MRV_SPARQL}, mrv_docs({"32023R2449": httpx.Response(400, text="bad")})
    )
    report = await ingest(db_session, client=client, topics=["mrv"], store=store)
...
async def test_an_aborted_run_records_no_result(db_session, store, corpus_client):
    client, _ = corpus_client({"mrv": httpx.Response(500, text="down")}, {})
    with pytest.raises(httpx.HTTPStatusError):
        await ingest(db_session, client=client, topics=["mrv"], store=store)
...
async def test_malformed_sparql_payload_raises_discovery_error(db_session, store, corpus_client):
...
async def test_run_persists_chunks_for_every_document(db_session, store, corpus_client):
    client, _ = corpus_client({"mrv": MRV_SPARQL}, mrv_docs())
    report = await ingest(db_session, client=client, topics=["mrv"], store=store)
...
async def test_second_identical_run_adds_and_removes_nothing(db_session, store, corpus_client):
    docs = mrv_docs()
    client, _ = corpus_client({"mrv": MRV_SPARQL}, docs)
    await ingest(db_session, client=client, topics=["mrv"], store=store)
...
async def test_dropped_document_loses_its_chunks(db_session, store, corpus_client):
    docs = mrv_docs()
    client, _ = corpus_client({"mrv": MRV_SPARQL}, docs)
    await ingest(db_session, client=client, topics=["mrv"], store=store)
...
async def test_a_plausible_repeal_still_prunes(db_session, store, corpus_client):
...
async def test_an_incomplete_run_prunes_nothing(db_session, store, corpus_client):
...
async def test_a_celex_another_topic_still_holds_survives_being_dropped(
    ...
    await ingest(db_session, client=client, topics=["fueleu"], store=store)
    ...
    report = await ingest(db_session, client=client, topics=["fueleu"], store=store)
...
async def test_a_row_whose_bytes_never_landed_is_recorded_not_raised(
    db_session, store, corpus_client, monkeypatch
):
    """A row is not evidence of an object: parse must survive one that points at nothing."""
    client, _ = corpus_client({"mrv": MRV_SPARQL}, mrv_docs())
    monkeypatch.setattr(stage, "_store", lambda *args: ("a" * 64, 0))
    report = await ingest(db_session, client=client, topics=["mrv"], store=store)
...
async def test_a_source_object_lost_from_storage_is_downloaded_again(
    db_session, store, corpus_client
):
    docs = mrv_docs()
    client, _ = corpus_client({"mrv": MRV_SPARQL}, docs)
    await ingest(db_session, client=client, topics=["mrv"], store=store)
    for path in store.root.rglob("*.html"):
        path.unlink()

    client, _ = corpus_client({"mrv": MRV_SPARQL}, docs)
    report = await ingest(db_session, client=client, topics=["mrv"], store=store)

    keys = {document.key for document in await get_corpus_docs(db_session)}
    assert all(store.exists(key) for key in keys)
    assert report.ok


async def test_a_storage_error_is_recorded_not_raised(
    db_session, store, corpus_client, monkeypatch
):
    def unwritable(*args, **kwargs):
        raise StorageError("put", "32015R0757/32015R0757/abc.html")

    monkeypatch.setattr(stage, "_store", unwritable)
    client, _ = corpus_client({"mrv": MRV_SPARQL}, mrv_docs())
    report = await ingest(db_session, client=client, topics=["mrv"], store=store)
...
async def test_partial_fetch_leaves_its_chunks_unstamped(db_session, store, corpus_client):
    client, _ = corpus_client(
        {"mrv": MRV_SPARQL}, mrv_docs({"32023R2449": httpx.Response(400, text="bad")})
    )
    report = await ingest(db_session, client=client, topics=["mrv"], store=store)
...
async def test_chunks_stay_attributed_to_the_run_that_first_stored_them(
    ...
    partial = await ingest(db_session, client=client, topics=["mrv"], store=store)
...
async def test_failed_fetch_still_chunks_what_was_downloaded(db_session, store, corpus_client):
    client, _ = corpus_client(
        {"mrv": MRV_SPARQL}, mrv_docs({"32023R2449": httpx.Response(400, text="bad")})
    )
    report = await ingest(db_session, client=client, topics=["mrv"], store=store)
...
async def test_a_reparse_reaches_no_network_at_all(db_session, store, corpus_client):
    """The done-when: a full reparse of the stored corpus, with fetch disabled."""
    client, _ = corpus_client({"mrv": MRV_SPARQL}, mrv_docs())
    first = await ingest(db_session, client=client, topics=["mrv"], store=store)

    client, calls = corpus_client({"mrv": httpx.Response(500, text="down")}, {})
    report = await ingest(db_session, client=client, topics=["mrv"], store=store, fetch=False)

    assert calls == []
    assert report.ok
    assert sorted(report.fetch.unchanged) == ["32015R0757", "32023R2449"]
    assert report.parse.parsed == first.parse.parsed


async def test_a_reparse_prunes_nothing_and_rechunks_from_the_stored_bytes(
    db_session, store, corpus_client
):
    client, _ = corpus_client({"mrv": MRV_SPARQL}, mrv_docs())
    first = await ingest(db_session, client=client, topics=["mrv"], store=store)
    before = len(await chunk_rows(db_session))
    ...
    assert len(await chunk_rows(db_session)) == before
    assert report.corpus_version == first.corpus_version


async def test_a_reparse_before_anything_was_fetched_aborts_the_run(
    db_session, store, corpus_client
):
    client, _ = corpus_client({}, {})
    with pytest.raises(EmptyCorpusError, match="mrv"):
        await ingest(db_session, client=client, topics=["mrv"], store=store, fetch=False)
...
async def ingest_fueleu(db_session, store, corpus_client) -> IngestRunResult:
    """Run the real pipeline over the saved FuelEU fixture, network stubbed."""
    client, _ = corpus_client(
        {"fueleu": FUELEU_SPARQL},
        {"32023R1805": httpx.Response(200, content=FUELEU_HTML.encode())},
    )
    return await ingest(db_session, client=client, topics=["fueleu"], store=store)


async def test_fueleu_chunks_have_correct_article_boundaries(db_session, store, corpus_client):
    report = await ingest_fueleu(db_session, store, corpus_client)
    assert report.ok
...
async def test_fueleu_chunks_carry_their_citations(db_session, store, corpus_client):
    await ingest_fueleu(db_session, store, corpus_client)
...
async def test_fueleu_annex_table_is_persisted_as_its_own_chunk(db_session, store, corpus_client):
    await ingest_fueleu(db_session, store, corpus_client)
...
async def test_fueleu_chunks_are_stamped_and_topic_tagged(db_session, store, corpus_client):
    report = await ingest_fueleu(db_session, store, corpus_client)
...
async def test_fueleu_chunk_count_matches_the_chunker(db_session, store, corpus_client):
...
async def test_a_run_embeds_every_chunk_it_stored(db_session, store, corpus_client):
    report = await ingest_fueleu(db_session, store, corpus_client)
...
async def test_a_second_run_embeds_nothing_and_reports_the_rest_unchanged(
    ...
    second = await ingest_fueleu(db_session, store, corpus_client)
